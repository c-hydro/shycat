...
def main():
    ...
    for time_now in forcing_period:
        ...
        if last_era != time_now.strftime("%Y"):
            ...
            for var_in in ["downward_radiation", "temperature", "wind", "total_precipitation", "relative_humidity"]:
                ...
                try:
                    ...
                    if var_in == "downward_radiation":    # nome del file ERA5
                        var_era = "downward_radiation"    # nome della variabile temporanea
                        var_erafile = "downward_radiation"    # nome della variabile nel file ERA5
                        era5_dset[var_era] = ds[var_erafile].load()

                    elif var_in == "temperature":
                        var_era = "temperature"
                        var_erafile = "temperature"
                        era5_dset[var_era] = ds[var_erafile].load()
                    ...
                    elif var_in == "total_precipitation":
                        var_era = "total_precipitation"
                        var_erafile = "total_precipitation"
                        era5_dset[var_era] = ds[var_erafile].load()

                    elif var_in == "relative_humidity":
                        var_era = "relative_humidity"
                        var_erafile = "relative_humidity"
                        era5_dset[var_era] = ds[var_erafile].load()
        ...
        maps["RH"] = np.nan_to_num(era5_dset["relative_humidity"].loc[time_now].values, nan=-9999)
        ...
        maps["rain"] = np.nan_to_num(era5_dset["total_precipitation"].loc[time_now].values, nan=-9999)

        product = "CHIRPS"
        print(" -----> Create forcing for time: " + time_now.strftime("%Y-%m-%d %H:%M") + " product: " + product)
